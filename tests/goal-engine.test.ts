...
test('maximum goals distinguish within from exceeded', () => {
  ...
  assert.equal(evaluateGoal(calories, 1800).status, 'within');
  assert.equal(evaluateGoal(calories, 2200).status, 'exceeded');
});

test('range goals distinguish below within and exceeded', () => {
  const carbs = goal({ mode: 'range', minimum: 150, maximum: 220 }, 'carbohydrate-g');
  assert.equal(evaluateGoal(carbs, 120).status, 'below');
  assert.equal(evaluateGoal(carbs, 180).status, 'within');
  assert.equal(evaluateGoal(carbs, 240).status, 'exceeded');
});

test('no goal stays informational', () => {
  const result = evaluateGoal(goal({ mode: 'none' }, 'fat-g'), 70);
  assert.equal(result.status, 'informational');
  assert.equal(result.ratio, null);
  assert.equal(result.remaining, null);
});

test('invalid range targets are rejected', () => {
  assert.throws(() => validateGoalTarget({ mode: 'range', minimum: 200, maximum: 100 }));
});
