...
export interface UsdaFoodRecord {
  ...
  foodNutrients?: ReadonlyArray<UsdaNutrientRecord>;
}
...
export function normalizeUsdaFood(record: UsdaFoodRecord, now: ISODateTime): Food {
  const source: NutritionSource = {
    ...
    recordId: String(record.fdcId) as SourceRecordId,
  };
  const nutrients: NutrientValue[] = (record.foodNutrients ?? []).map((entry) => ({
    nutrient: nutrientDefinition(entry),
    state: entry.amount === undefined ? 'unknown' : 'known',
    ...(entry.amount === undefined ? {} : { value: entry.amount }),
    source,
  }));
  ...
  return {
    id,
    kind: record.dataType === 'Branded' ? 'branded' : 'generic',
