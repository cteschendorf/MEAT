...
export interface TodaySnapshot {
  ...
  metrics: ReadonlyArray<TodayMetric>;
}
...
async function foodsForMeals(meals: ReadonlyArray<Meal>, repo: FoodRepository): Promise<Map<string, Food>> {
  const ids = [...new Set(meals.flatMap((meal) => meal.items.map((item) => item.foodId)))];
  const values = await Promise.all(ids.map((id) => repo.getById(id)));
  const map = new Map<string, Food>();
  values.forEach((food) => {
    if (food) map.set(food.id, food);
  });
  return map;
}
